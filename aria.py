#
# CS448 Project: ARIA
# Group	12
#
# 2015. 05. 29.
#




S = (#=> S[4][256]
#S-box type 1
(
  ),
  # S-box type 2
(
  ),
  # inverse of S-box type 1
(
  ),
  # inverse of S-box type 2
(
  )
)

# constants used in the round key generation
KRK = (#=>[3][16]
  (0x51, 0x7c, 0xc1, 0xb7, 0x27, 0x22, 0x0a, 0x94, 0xfe, 0x13, 0xab, 0xe8, 0xfa, 0x9a, 0x6e, 0xe0), 
  (0x6d, 0xb1, 0x4a, 0xcc, 0x9e, 0x21, 0xc8, 0x20, 0xff, 0x28, 0xb1, 0xd5, 0xef, 0x5d, 0xe2, 0xb0), 
  (0xdb, 0x92, 0x37, 0x1d, 0x21, 0x26, 0xe9, 0x70, 0x03, 0x24, 0x97, 0x75, 0x04, 0xe8, 0xc9, 0x0e)
)

# Diffusion Layer
def DL (i,o): #void DL (const Byte *i, Byte *o)
	#Byte T  
	T = i[ 3] ^ i[ 4] ^ i[ 9] ^ i[14]
	o[ 0] = i[ 6] ^ i[ 8] ^ i[13] ^ T
	o[ 5] = i[ 1] ^ i[10] ^ i[15] ^ T
	o[11] = i[ 2] ^ i[ 7] ^ i[12] ^ T
	o[14] = i[ 0] ^ i[ 5] ^ i[11] ^ T
	T = i[ 2] ^ i[ 5] ^ i[ 8] ^ i[15]
	o[ 1] = i[ 7] ^ i[ 9] ^ i[12] ^ T
	o[ 4] = i[ 0] ^ i[11] ^ i[14] ^ T
	o[10] = i[ 3] ^ i[ 6] ^ i[13] ^ T
	o[15] = i[ 1] ^ i[ 4] ^ i[10] ^ T
	T = i[ 1] ^ i[ 6] ^ i[11] ^ i[12]
	o[ 2] = i[ 4] ^ i[10] ^ i[15] ^ T
	o[ 7] = i[ 3] ^ i[ 8] ^ i[13] ^ T
	o[ 9] = i[ 0] ^ i[ 5] ^ i[14] ^ T
	o[12] = i[ 2] ^ i[ 7] ^ i[ 9] ^ T
	T = i[ 0] ^ i[ 7] ^ i[10] ^ i[13]
	o[ 3] = i[ 5] ^ i[11] ^ i[14] ^ T
	o[ 6] = i[ 2] ^ i[ 9] ^ i[12] ^ T
	o[ 8] = i[ 1] ^ i[ 4] ^ i[15] ^ T
	o[13] = i[ 3] ^ i[ 6] ^ i[ 8] ^ T

# Right-rotate 128 bit source string s by n bits and XOR it to target string t
def RotXOR (s, n, t, idx): #void RotXOR (const Byte *s, int n, Byte *t)
	#int i, q;
	q = n/8
	n %= 8
	for i in range(16):
		t[(q+i) % 16+idx] ^= (s[i] >> n)
		if (n != 0):
			t[(q+i+1) % 16+idx] ^= (s[i] << (8-n))

def tri_op(q): #q = (q==2)? 0 : (q+1)
	if q==2:
		q =	0
	else:
		q+= 1
	return q

# Encryption round key generation rountine
# w0 : master key, e : encryption round keys
def EncKeySetup(w0, e, keyBits): #int EncKeySetup(const Byte *w0, Byte *e, int keyBits)
	R=(keyBits+256)/32
	#Byte t[16], w1[16], w2[16], w3[16];
	t = []
	w1 = []  	
	w2 = []  	
	w3 = []
  	
	for i in range(16):
		t.append(0)
		w1.append(0)  		
		w2.append(0)  		
		w3.append(0)

	q = (keyBits - 128) / 64
	for i in range(16): t[i] = S[	   i % 4][KRK[q][i] ^ w0[i]]
	DL (t, w1)
	if (R==14):
		for i in range(8): w1[i] ^= w0[16+i]
	elif (R==16):
		for i in range(16): w1[i] ^= w0[16+i]

	q = tri_op(q)
	for i in range(16): t[i] = S[(2 + i) % 4][KRK[q][i] ^ w1[i]]
	DL (t, w2)
	for i in range(16): w2[i] ^= w0[i]
	
	q = tri_op(q)
	for i in range(16): t[i] = S[     i  % 4][KRK[q][i] ^ w2[i]]
	DL (t, w3)
	for i in range(16): w3[i] ^= w1[i]

	for i in range(16*(R+1)): e[i] = 0

	RotXOR (w0, 0, e ,	0 ); RotXOR (w1,  19, e ,   0);
	RotXOR (w1, 0, e ,  16); RotXOR (w2,  19, e ,  16);
	RotXOR (w2, 0, e ,  32); RotXOR (w3,  19, e ,  32);
	RotXOR (w3, 0, e ,  48); RotXOR (w0,  19, e ,  48);
	RotXOR (w0, 0, e ,  64); RotXOR (w1,  31, e ,  64);
	RotXOR (w1, 0, e ,  80); RotXOR (w2,  31, e ,  80);
	RotXOR (w2, 0, e ,  96); RotXOR (w3,  31, e ,  96);
	RotXOR (w3, 0, e , 112); RotXOR (w0,  31, e , 112);
	RotXOR (w0, 0, e , 128); RotXOR (w1,  67, e , 128);
	RotXOR (w1, 0, e , 144); RotXOR (w2,  67, e , 144);
	RotXOR (w2, 0, e , 160); RotXOR (w3,  67, e , 160);
	RotXOR (w3, 0, e , 176); RotXOR (w0,  67, e , 176);
	RotXOR (w0, 0, e , 192); RotXOR (w1,  97, e , 192);
	if (R > 12): 
		RotXOR (w1, 0, e , 208); RotXOR (w2,  97, e , 208);
		RotXOR (w2, 0, e , 224); RotXOR (w3,  97, e , 224);
	
	if (R > 14): 
		RotXOR (w3, 0, e , 240); RotXOR (w0,  97, e , 240);
		RotXOR (w0, 0, e , 256); RotXOR (w1, 109, e , 256);
	return R

# Decryption round key generation rountine
# w0 : maskter key, d : decryption round keys
def DecKeySetup(w0, d, keyBits): #int DecKeySetup(const Byte *w0, Byte *d, int keyBits)
	#int  i, j, R;
	#Byte t[16];
  
	R = EncKeySetup(w0, d, keyBits)
	for j in range(16):
		t[j] = d[j]
		d[j] = d[16*R + j]
		d[16*R + j] = t[j]

	for i in range(1,R/2+1):
		DL (d + i*16, t)
		DL (d + (R-i)*16, d + i*16)
		for j in range(16): d[(R-i)*16 + j] = t[j]
	return R

# Encryption and decryption rountine
# p: plain text, e: round keys, c: ciphertext
def Crypt(p, R, e, c): #void Crypt(const Byte *p, int R, const Byte *e, Byte *c)

	#int i, j;
	#Byte t[16];
	t = []
	for i in range(16): t.append(0)
	
	for j in range(16): c[j] = p[j]
	
	for i in range(R/2):
		for j in range(16): t[j] = S[     j  % 4][e[j] ^ c[j]]
		DL(t, c);
		for j in range(16): t[j] = S[(2 + j) % 4][e[j+16] ^ c[j]]
		DL(t, c);
	DL(c, t)
	for j in range(16): c[j] = e[j+16] ^ t[j]

def printBlockOfLength(b,len): #void printBlockOfLength(Byte *b, int len) 
	#int i;
  	for i in range(len):
  		print("%02x" %b)
  		if (i%4==3 & i<len-1): 
  			print(" ")
  		b+=1
	
def printBlock(b): #void printBlock(Byte *b)
	printBlockOfLength(b, 16)

def ARIA_test():
  	#Byte rk[16*17], c[16], mk[32];
  	rk = []
  	for i in range(16*17): rk.append(0)
  	
  	c = []
  	for i in range(16): c.append(0)

  	mk = []
  	for i in range(32): mk.append(0)

  	p = (0x11, 0x11, 0x11, 0x11, 0xaa, 0xaa, 0xaa, 0xaa, 
    0x11, 0x11, 0x11, 0x11, 0xbb, 0xbb, 0xbb, 0xbb) #16bytes
  	cryptResult = (
    0x8d, 0x14, 0x70, 0x62, 0x5f, 0x59, 0xeb, 0xac,
    0xb0, 0xe5, 0x5b, 0x53, 0x4b, 0x3e, 0x46, 0x2b) 
	#int i, flag;
	
  	for i in range(16): mk[i] = i * 0x11
  	for i in range(16,24): mk[i] = (i-16) * 0x11
    Crypt(p,EncKeySetup(mk,rk,192),rk,c)

    print("BEGIN testing basic encryption...\n");
	print("Testing whether the encryption would come out correctly, \
	for 14-round ARIA.\n");
	print("key      : "); printBlockOfLength(mk, 24); print("\n");
	print("plaintext: "); printBlock(p); print("\n");
	print("result is: "); printBlock(c); print("\n");
	print("should be: "); printBlock(cryptResult); print("\n");
	flag=0
	for i in range(16):
		if (c[i]!=cryptResult[i]):
	   		flag=1
	if (flag==1):
	  	print("The result is incorrect!\n")
	else:
		print("Okay.  The result is correct.\n")
	print("END   testing basic encryption.\n\n")
	  
	for i in range(32):
		mk[i]=0
	  
	for i in range(16):
		p[i]=0
	  
	print("BEGIN testing the roundtrip...\n")
	print("For key size of 256 bits, starting with \
	the zero plaintext and the zero key, let's see if \
	we may recover the plaintext by decrypting the \
	encrypted ciphertext.\n")
	EncKeySetup(mk, rk, 256)
	print("plaintext : "); printBlock(p); print("\n");
	Crypt(p, 16, rk, c)
	print("ciphertext: "); printBlock(c); print("\n")
	DecKeySetup(mk, rk, 256)
	Crypt(c, 16, rk, p)
	print("decrypted : "); printBlock(p); print("\n");
	flag=0
	for i in range(16):
	  	if (p[i]!=0):
	  		flag=1
	if (flag==1):
		print("The result is incorrect!\n")
	else:
		print("Okay.  The result is correct.\n")
	print("END   testing the roundtrip.\n")


def main(): 
	ARIA_test()
	return 0

main()










